...
mod rule {
    ...
    impl SuricataRule {
        pub fn matches(&self, pkt: &ParsedPacket) -> bool {
            if !self.proto.matches(pkt.l4) {
                return false;
            }
            if !self.src.matches(pkt.src_ip) {
                return false;
            }
            if !self.dst.matches(pkt.dst_ip) {
                return false;
            }
...
mod loader {
    ...
    fn parse_rule(s: &str) -> Result<SuricataRule, String> {
        ...
        if parts.len() < 7 {
            return Err(format!("short header: {s}"));
        }
...
impl SuricataRuleSet {
    pub fn new(rules: Vec<SuricataRule>) -> Self {
        Self { rules }
    }
