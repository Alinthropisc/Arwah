...
impl DpiEngine {
    pub fn default_set() -> Self {
        Self {
            detectors: vec![
                ...
                Box::new(DnsDetector),
                Box::new(SshDetector),
                Box::new(QuicDetector),
                Box::new(WireGuardDetector),
            ],
        }
    }
    ...
    pub fn inspect(&self, payload: &[u8], pkt: &ParsedPacket) -> DpiResult {
        for d in &self.detectors {
            if let Some(r) = d.probe(payload, pkt) {
                return r;
            }
        }
        DpiResult::Unknown
...
mod tests {
    ...
    use b579_core::{
        packet::ParsedPacket,
        protocol::{AppProtocol, L3Protocol},
    };
    use chrono::Utc;
