...
impl AfPacketCapture {
    pub fn open(interface: &str) -> ArwahResult<Self> {
        ...
        let iface =
            CString::new(interface).map_err(|_| ArwahError::InvalidInterface(interface.into()))?;
        let idx = unsafe { libc::if_nametoindex(iface.as_ptr()) };
        if idx == 0 {
            unsafe { libc::close(fd) };
            return Err(ArwahError::InvalidInterface(interface.into()));
        }
        ...
        if rc < 0 {
            unsafe { libc::close(fd) };
            return Err(ArwahError::Capture("bind() failed".into()));
        }
...
impl CaptureSource for AfPacketCapture {
    ...
    fn set_bpf_filter(&mut self, _: &str) -> ArwahResult<()> {
        Ok(())
    }
